...
{
    public partial class StbColumn : IModelElement_210
    {
        [XmlIgnore]
        ...
        List<IPropertyTab> IModelElement.GetAdditionalDetails(IST_BRIDGE istBridge)
        {
            ...
            if (joint_id_bottom != null || joint_id_top != null)
            {
                ...
                if (joint_id_bottom != null)
                {
                    ...
                    if (shapeH != null)
                    {
                        jointProperties.AddRange(IModelElement_210.GetPropertyDetail(shapeH));
                    }

                    StbJointColumnShapeT? shapeT = stBridge.StbModel.StbJoints?.StbJointColumnShapeT.FirstOrDefault(j => j.id == joint_id_bottom);
                    if (shapeT != null)
                    {
                        jointProperties.AddRange(IModelElement_210.GetPropertyDetail(shapeT));
                    }
                    ...
                    if (crossH != null)
                    {
                        jointProperties.AddRange(IModelElement_210.GetPropertyDetail(crossH));
                    }
                }
